...
namespace TheGuideToTheNewEden.ActiveWindow
{
    internal class HotkeyService
    {
        ...
        {
            Console.WriteLine("开始监听按键");
            if(Current._keyboardHook == null)
            {
                Current._keyboardHook = new KeyboardHook();
            }
            if(Current._keyboardHook.Start())
            {
                Console.WriteLine("安装钩子成功");
                Current._keyboardHook.KeyboardEvent += _keyboardHook_KeyboardEvent1;
            }
            else
            {
                Console.WriteLine("安装钩子失败");
            }
        }

        private static void _keyboardHook_KeyboardEvent1(List<KeyboardInfo> keys)
        {
            Console.WriteLine($"HotkeyService监听到按键{keys.Count}");
            OnKeyboardClicked?.Invoke(keys);
        }

        public static void Stop()
        {
            Console.WriteLine("停止监听按键");
            ...
            if (nCode == 0)
            {
                ...
                if (wParam == 0x100)//按下
                {
                    ...
                    if (!pressedKeyDic.ContainsKey(vk))
                    {
                        ...
                        Console.WriteLine($"捕获到新按下按键{name}");
                ...
                {
                    Console.WriteLine($"松开{vk}");
                    if (pressedKeyDic.Remove(vk))
                    {
                        Console.WriteLine($"捕获到松开已有按键{vk}");
                        KeyboardEvent?.Invoke(pressedKeyDic.Values.ToList());
                    }
                }
            }
