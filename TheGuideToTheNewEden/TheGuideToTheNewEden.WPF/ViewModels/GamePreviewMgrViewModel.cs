...
using TheGuideToTheNewEden.WPF.Wins.GamePreviewWindows;
...
namespace TheGuideToTheNewEden.WPF.ViewModels
{
    internal class GamePreviewMgrViewModel : BaseViewModel
    {
        ...
        private Window _unvisibleWindow;
        ...
        private bool RegisterHotkey(string hotkey, out int hotkeyRegisterId)
        {
            ...
            {
                if (HotkeyService.GetHotkeyService(WindowHelper.GetWindowHandle(Window)).Register(hotkey, out hotkeyRegisterId))
                {
                    HotkeyService.GetHotkeyService(WindowHelper.GetWindowHandle(Window)).HotkeyActived -= GamePreviewMgrViewModel_HotkeyActived;
                    HotkeyService.GetHotkeyService(WindowHelper.GetWindowHandle(Window)).HotkeyActived += GamePreviewMgrViewModel_HotkeyActived;
                    return true;
                }
                else
        ...
        private bool UnregisterForwardHotkey()
        {
            ...
            {
                if (HotkeyService.GetHotkeyService(WindowHelper.GetWindowHandle(Window)).Unregister(_forwardHotkeyRegisterId))
                {
        ...
        private bool UnregisterBackwardHotkey()
        {
            ...
            {
                if (HotkeyService.GetHotkeyService(WindowHelper.GetWindowHandle(Window)).Unregister(_backwardHotkeyRegisterId))
                {
        ...
        private bool Start(ProcessInfo processInfo, PreviewItem setting, PreviewSetting previewSetting)
        {
            ...
            {
                if(_unvisibleWindow == null)
                {
                    _unvisibleWindow = new Window()
                    {
                        WindowStyle = WindowStyle.ToolWindow,
                        ShowInTaskbar = false,
                        Title = "GamePreviewMgrWindow",
                    };
                    _unvisibleWindow.SourceInitialized += UnvisibleWindow_SourceInitialized;
                    _unvisibleWindow.Show();
                }
                
                try
                {
                    IGamePreviewWindow gamePreviewWindow = null;
                    if (setting.ShowPreviewWindow)
                    {
                        switch (setting.ShowPreviewWindowMode)
                        {
                            default: gamePreviewWindow = new GamePreviewWindow1(setting, previewSetting); break;
                        }
                    }
                    else
                    {
                    ...
                    gamePreviewWindow.SetParentWindow(_unvisibleWindow);
        ...
        private void UnvisibleWindow_SourceInitialized(object sender, EventArgs e)
        {
            var hwnd = WindowHelper.GetWindowHandle(_unvisibleWindow);
            Win32.SetWindowPos(hwnd, 0, -100000, -100000, 0, 0, 0x0004);
        }
