...
/// <summary>情报红圈标记。</summary>
public sealed class IntelMarker
{
    public int SystemId { get; init; }
    /// <summary>威胁权重（舰船数 + 频道消息数），决定圆圈大小。</summary>
...
/// 深空背景 + 视差星尘 + 星门连线 + 发光星系节点（按安等/击杀/通行量着色），
/// 情报红圈脉冲、角色头像标记、航线发光折线与流动光点。
...
public class StarMapCanvas : SKElement
{
    ...
    // ---------- 绘制资源 ----------
    private readonly SKTypeface _typeface = SKTypeface.FromFamilyName("Segoe UI", SKFontStyle.Normal)
        ?? SKTypeface.Default;
    private readonly SKTypeface _typefaceBold = SKTypeface.FromFamilyName("Segoe UI", SKFontStyle.Bold)
        ?? SKTypeface.Default;
    ...
    public void SetData(IReadOnlyList<MapSystemNode> nodes, IEnumerable<(int From, int To)> links)
    {
        ...
        {
            _indexById[node.Id] = _indexById.Count; // 占位，稍后重建
    ...
    private void OnPaintSurface(object? sender, SKPaintSurfaceEventArgs e)
    {
        ...
        canvas.Clear(new SKColor(4, 6, 12));
    ...
    /// <summary>重建底图缓存（背景 + 星尘 + 连线 + 全部节点与标签）。</summary>
    private void RebuildBase(int deviceW, int deviceH, float w, float h, double zmult, float nodeR)
    {
        ...
        using var baseCanvas = new SKCanvas(_baseCache);
    ...
    private void DrawBackground(SKCanvas canvas, SKPaint paint, float w, float h, double zmult)
    {
        ...
        using var bg = SKShader.CreateRadialGradient(
            ...
            [new SKColor(13, 20, 42), new SKColor(6, 9, 20), new SKColor(3, 4, 10)],
    ...
    private void DrawStarLayer(SKCanvas canvas, SKPaint paint, List<(double x, double y, float r, byte a, SKColor c)> stars, double parallax, double zmult)
    {
        ...
        foreach (var (x, y, r, a, c) in stars)
        {
            ...
            paint.Color = new SKColor(c.Red, c.Green, c.Blue, a);
    ...
    {
        // 连线透明度随缩放淡入
        var alpha = (float)Math.Clamp((zmult - 1.05) * 55, 0, 80);
        if (alpha <= 1)
        {
            return;
        }
    ...
    private void DrawNodes(SKCanvas canvas, SKPaint paint, SKFont font, double zmult, float nodeR)
    {
        ...
        foreach (var node in _nodes)
        {
            ...
            if (zmult > 30)
            {
                paint.Color = new SKColor(255, 255, 255, (byte)Math.Clamp((zmult - 30) * 4, 0, 160));
                canvas.DrawCircle(p, nodeR * 0.45f, paint);
            }

            if (showSec)
            {
                var secText = FormatSecurity(node.Security);
                font.Typeface = _typefaceBold;
                font.Size = secTextSize;
                var width = font.MeasureText(secText);
                paint.Color = new SKColor(10, 14, 24, 210);
                canvas.DrawCircle(p, secTextSize * 0.85f + 2.5f, paint);
                paint.Color = new SKColor(235, 242, 255, 230);
            ...
            if (showName)
            {
                ...
                paint.Color = new SKColor(205, 220, 245, (byte)Math.Clamp(60 + zmult * 12, 80, 235));
    ...
    private SKImage GetGlowImage(SKColor color)
    {
        ...
        using (var surface = new SKCanvas(bitmap))
        {
            ...
            p.Shader = SKShader.CreateRadialGradient(
                ...
                [new SKColor(color.Red, color.Green, color.Blue, 255), SKColors.Transparent],
    ...
    private void DrawRoute(SKCanvas canvas, SKPaint paint, double zmult, float nodeR)
    {
        ...
        paint.Color = new SKColor(125, 249, 255, 45);
        canvas.DrawPath(path, paint);
        // 主线
        paint.StrokeWidth = Math.Max(1.4f, nodeR * 0.45f);
        paint.Color = new SKColor(125, 249, 255, 200);
        canvas.DrawPath(path, paint);
        path.Dispose();
        ...
        paint.Style = SKPaintStyle.Fill;
        paint.Color = new SKColor(220, 255, 255, 230);
    ...
    private void DrawIntel(SKCanvas canvas, SKPaint paint, float nodeR)
    {
        foreach (var marker in _intel)
        {
            ...
            glow.Shader = SKShader.CreateRadialGradient(p, radius * 1.8f,
                [new SKColor(255, 59, 80, (byte)(70 + pulse * 50)), SKColors.Transparent],
            ...
            paint.Color = new SKColor(255, 72, 92, (byte)(150 + pulse * 100));
            canvas.DrawCircle(p, radius, paint);

            paint.StrokeWidth = 1f;
            paint.Color = new SKColor(255, 72, 92, 90);
    ...
    private void DrawCharacters(SKCanvas canvas, SKPaint paint, float nodeR, double zmult)
    {
        foreach (var marker in _characters)
        {
            ...
            paint.Color = new SKColor(88, 196, 255, 220);
            canvas.DrawCircle(p.X, cy, size * 0.68f + 1.6f, paint);
            paint.StrokeWidth = 1f;
            paint.Color = new SKColor(88, 196, 255, 110);
            ...
            {
                paint.Color = new SKColor(88, 196, 255, 235);
            ...
            if (zmult > 2.2)
            {
                ...
                paint.Color = new SKColor(160, 214, 255, 230);
    ...
    private void DrawHover(SKCanvas canvas, SKPaint paint, float nodeR)
    {
        ...
        paint.Color = new SKColor(255, 255, 255, 210);
        ...
        {
            paint.StrokeWidth = 1.6f;
            paint.Color = new SKColor(150, 235, 255, 190);
    ...
    private void DrawSelection(SKCanvas canvas, SKPaint paint, float nodeR)
    {
        ...
        paint.Color = new SKColor(125, 249, 255, 110);
    ...
    private void DrawHighlight(SKCanvas canvas, SKPaint paint, float nodeR)
    {
        ...
        if (_indexById.TryGetValue(_highlight.Id, out var index))
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                ...
                paint.StrokeWidth = 1.5f;
                paint.Color = new SKColor(125, 249, 255, (byte)(255 * (1 - t)));
    ...
    /// <summary>切换着色模式并重算节点颜色（热度模式使用节点 Heat 值）。</summary>
    public void SetColorMode(MapColorMode mode, double killsMax = 0, double jumpsMax = 0)
    {
    ...
    /// <summary>使底图缓存失效（数据/视图变化时调用）。</summary>
    private void InvalidateBase() => _baseKey = null;
